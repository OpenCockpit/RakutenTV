...
class PiconFetcher:
    def __init__(self, parent=None):
        ...
        self.pluginPiconDir = os.path.join(self.piconDir, "RakutenTV")
        piconWidth = 220
        piconHeight = 132
    ...
    def addPicon(self, ref, name, url, silent):
        if not config.plugins.rakutentv.picons.value:
            return
        piconname = os.path.join(self.piconDir, ch_name + ".png") if config.plugins.rakutentv.picons.value == "snp" and (ch_name := sanitizeFilename(name.lower())) else os.path.join(self.piconDir, ref.replace(":", "_") + ".png")
        ...
        if self.piconList:
            ...
            for thread in picon_threads:
                if thread not in failed:
                    thread.join()
            print("[RakutenTV PiconFetcher] all fetched")
