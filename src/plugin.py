...
from . import _
from .RakutenTVConfig import getselectedregions, NUMBER_OF_LIVETV_BOUQUETS
...
from .RakutenTVUtils import resumePointsInstance, downloadPoster
from .Variables import TIMER_FILE, PLUGIN_FOLDER, BOUQUET_FILE, PLUGIN_ICON


class RakutenList(MenuList):
    def __init__(self, entries):
        self.menu_png = LoadPixmap(x if fileExists(x := resolveFilename(SCOPE_CURRENT_SKIN, "icons/rakuten_menu.png")) else f"{PLUGIN_FOLDER}/images/menu.png") if fileExists(f"{PLUGIN_FOLDER}/images/menu.png") else None
        self.cine_png = LoadPixmap(x if fileExists(x := resolveFilename(SCOPE_CURRENT_SKIN, "icons/rakuten_cine.png")) else f"{PLUGIN_FOLDER}/images/cine.png")
        self.cine_half_png = LoadPixmap(x if fileExists(x := resolveFilename(SCOPE_CURRENT_SKIN, "icons/rakuten_cine_half.png")) else f"{PLUGIN_FOLDER}/images/cine_half.png")
        self.cine_end_png = LoadPixmap(x if fileExists(x := resolveFilename(SCOPE_CURRENT_SKIN, "icons/rakuten_cine_end.png")) else f"{PLUGIN_FOLDER}/images/cine_end.png")
...
class RakutenTV(Screen, HelpableScreen):
    skin = f"""
        <screen name="RakutenTV" zPosition="2" position="0,0" resolution="1920,1080" size="1920,1080" flags="wfNoBorder" title="Rakuten TV" transparent="0">
            <ePixmap pixmap="{PLUGIN_FOLDER}/images/rakutentv-backdrop.jpg" position="0,0" size="1920,1080" zPosition="-2" alphatest="off" />
    ...
    def __init__(self, session):
        ...
        self["actions"] = HelpableActionMap(
            self, ["SetupActions", "InfobarChannelSelection", "MenuActions"],
            {
                ...
                "save": (self.green, _("Create or update Rakuten TV live bouquets")),
    ...
    def initialise(self):
        self.region = config.plugins.rakutentv.region.value
        self.films = []
        self.menu = []
        self.history = []
        self.vinfo = ""
        ...
        self["vtitle"].setText("")
        self["loading"].show()
        self.title = _("Rakuten TV") + " - " + self.titlemenu
    ...
    def getCategories(self):
        self.lvod = {}
        categories = rakutenRequest.getVODCategories(self.region)
        if not categories:
    ...
    def buildlist(self, category):
        ...
        for item in items:
            ...
            self.lvod[name].append((itemid, itemname, itemsummary, itemgenre, itemrating, itemduration, itemposter, itemimage, itemtype, stream_url, item.get("language_id", "ENG")))
...
class RakutenSetup(Setup):
    ...
    def createSetup(self):
        configList = []
        configList.append((_("Region"), config.plugins.rakutentv.region, _("Select the region for the channel list.")))
        configList.append(("---",))
        for n in range(1, NUMBER_OF_LIVETV_BOUQUETS + 1):
            if n == 1 or getattr(config.plugins.rakutentv, "live_tv_region" + str(n - 1)).value:
                configList.append((_("LiveTV bouquet %s") % n, getattr(config.plugins.rakutentv, "live_tv_region" + str(n)), _("Region for which LiveTV bouquet %s will be created.") % n))
        configList.append(("---",))
        ...
        configList.append((_("Data location"), config.plugins.rakutentv.datalocation, _("Used for storing video cover graphics, etc.")))
...
class Rakuten_Player(MoviePlayer):
